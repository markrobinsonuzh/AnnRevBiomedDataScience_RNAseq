...
legend <- get_legend(p1b)
# align all plots vertically
plots <- align_plots(p1b + theme(legend.position="none", axis.title.y=element_blank()), 
                     p3 + theme(legend.position="none", axis.title.y=element_blank()), 
                     p5 + theme(legend.position="none", axis.title.y=element_blank()), 
                     align = 'v', 
                     axis = 'l')
top_row <- plot_grid(plots[[1]], plots[[2]], 
                     labels = c("A", "B"), 
                     rel_widths = c(1,1), 
                     nrow = 1)
bottom_row <- plot_grid(plots[[3]], legend, 
                        labels = c("C", ""), 
                        rel_widths = c(1,1), 
                        nrow = 1)
pg2<- plot_grid(top_row, bottom_row, ncol = 1)
pg2
```
